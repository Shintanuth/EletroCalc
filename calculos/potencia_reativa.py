...
from util.conversores import seno_graus, tangente_graus
...
def calcular_potencia_reativa_por_tensao_corrente_angulo(tensao, corrente, angulo):
    ...
    return (
        "Potência Reativa",
        tensao * corrente * (seno_graus(angulo)),
        VOLT_AMPERE_REATIVO,
    )


def calcular_potencia_reativa_por_potencia_aparente_potencia_ativa(
    ...
    validar_maior(
        potencia_ativa, potencia_aparente, ERRO_POTENCIA_ATIVA_MAIOR_QUE_APARENTE
    )

    return (
        "Potência Reativa",
        sqrt((potencia_aparente**2) - (potencia_ativa**2)),
        VOLT_AMPERE_REATIVO,
    )
...
def calcular_potencia_reativa_por_potencia_ativa_angulo(potencia_ativa, angulo):
    ...
    return (
        "Potência Reativa",
        potencia_ativa * (tangente_graus(angulo)),
        VOLT_AMPERE_REATIVO,
    )
